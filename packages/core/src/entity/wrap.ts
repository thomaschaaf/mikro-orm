 ...
 */
export function wrap<T>(entity: T, preferHelper: true): IWrappedEntityInternal<T>;

/**
 * wraps entity type with WrappedEntity internal properties and helpers like init/isInitialized/populated/toJSON
 */
export function wrap<T>(entity: T, preferHelper?: false): IWrappedEntity<T>;

/**
 * wraps entity type with WrappedEntity internal properties and helpers like init/isInitialized/populated/toJSON
 * use `preferHelper = true` to have access to the internal `__` properties like `__meta` or `__em`
 */
export function wrap<T>(entity: T, preferHelper = false): IWrappedEntity<T> | IWrappedEntityInternal<T> {
  if (entity instanceof BaseEntity && !preferHelper) {
    return entity as IWrappedEntity<T>;
  }

  if (entity instanceof ArrayCollection) {
    return entity as unknown as WrappedEntity<T>;
  }
