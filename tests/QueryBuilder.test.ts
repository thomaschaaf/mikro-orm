...
describe('QueryBuilder', () => {
  ...
  test('select by m:n inverse side (that is not defined as property) via populate', async () => {
    ...
    qb.select('*').populate([{ field: 'publisher2_tests' as any }]).where({ 'publisher2_tests.Publisher2_owner': { $in: [ 1, 2 ] } }).orderBy({ 'publisher2_tests.id': QueryOrder.ASC });
  ...
  test('select by m:n self reference owner', async () => {
    const qb = orm.em.createQueryBuilder(Author2);
    qb.select('*').populate([{ field: 'author2_following' as any }]).where({ 'author2_following.Author2_owner': { $in: [ 1, 2 ] } });
  ...
  test('select by m:n self reference inverse', async () => {
    const qb = orm.em.createQueryBuilder(Author2);
    qb.select('*').populate([{ field: 'author2_following' as any }]).where({ 'author2_following.Author2_inverse': { $in: [ 1, 2 ] } });
  ...
  test('select by m:n with composite keys', async () => {
    ...
    qb.select('*').populate([{ field: 'user2_cars' as any }]).where({ 'user2_cars.Car2_inverse': { $in: [ [1, 2], [3, 4] ] } });
  ...
  test('select by m:n with unknown populate ignored', async () => {
    const qb = orm.em.createQueryBuilder(Test2);
    qb.select('*').populate([{ field: 'not_existing' as any }]);
