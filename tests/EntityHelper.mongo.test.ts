...
describe('EntityAssignerMongo', () => {
  ...
  test('#toJSON properly calls child entity toJSON with correct params', async () => {
    ...
    const newGod = (await orm.em.findOne(Author, god.id, { populate: ['books.author'] }))!;
  ...
  test('#toObject complex serialization (1:m -> m:1)', async () => {
    ...
    const author = (await orm.em.findOne(Author, god.id, { populate: ['favouriteAuthor', 'books.author', 'books.publisher'] }))!;
